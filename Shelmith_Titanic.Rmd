...
```{r}
titanic_imputation<- mice(titanic_dataset[, c("Age","Embarked","Fare")], method = "cart", seed = 201,m = 5)

titanic_imputation <- complete(titanic_imputation, sample(c(1:5), 1))
...
```{r}
train.dataset.rose <- ROSE(Survived ~ ., data = train.dataset2[,-1], seed = seed_object)$data
...
write.csv(titanic_submission,"titanic_submission_glmnet.csv")
...
titanic_submission<-test.dataset2.rose_xgb%>%
  dplyr::select(PassengerId,Survived)
write.csv(titanic_submission,"titanic_submission_xgb.csv")
